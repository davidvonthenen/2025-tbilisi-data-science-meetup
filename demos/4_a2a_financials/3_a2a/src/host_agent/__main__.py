...
async def get_response_from_agent(
    ...
    # IMPORTANT:
    # ChatInterface treats generator yields as progressive updates to a *single* bot bubble.
    # If we yield multiple strings, only the last is visible. Aggregate everything first.
    combined = "\n\n".join(r for r in responses if r).strip() or "I don't know."
    yield gr.ChatMessage(role="assistant", content=combined)
...
async def main() -> None:
    ...
    with gr.Blocks(title="A2A Host Agent") as demo:
        ...
        gr.ChatInterface(
            ...
            description=(
                "This assistant routes your prompt to a News Agent or a Financial Agent.\n"
                "Non-financial questions → News. Financial questions require a recognizable ticker "
                "(e.g., $MSFT, $GOOG, etc) to route to Finance."
            ),
        )
